...
function buildApiUrl(apiPath) {
    // The Marvel API can be used with a single public key when running on a web server, otherwise it needs to use
    // two keys. The public key only would work fine if running via GitHub pages, or a localhost web server, but
    // not when loading file directly. So we need to use both for now.
    const privateKey = '2f20f804b3062e2dba696aa4a4f4cbb9f54fcd17';
    const publicKey = "2abb8d4dbef38b7b61728089ea5eb10e";

    let ts = Date.now();
    let hash = md5(ts + privateKey + publicKey);
    return `https://gateway.marvel.com/v1/public/${apiPath}?ts=${ts}&apikey=${publicKey}&hash=${hash}`;
}
